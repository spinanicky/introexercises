...
Open the R project, as usual, and your script "ebola_analysis.R". Create a new section below the other code, well documented with # symbols, for these exercises to create simple plots.  
...
The {ggplot2} package does not need to be listed explicitly, because it is included in the {tidyverse} mega-package (which should already be in your list of packages).  
...
That is not the plot we want! How to fix this?  
...
Now run the command. It should look exactly the same as the default `geom_col()` command, when used on the `surv` dataset.  
...
Try adjusting the `fill =` aesthetic. Can you assign it dynamically to another column? Which columns are available to you, in this new data frame?  
...
1) We have *assigned* the ggplot command a name ("height_plot") and saved it as an object. Look, it appears in the R Environment now.  

2) A second command uses the {ggExtra} function `ggMarginal()`. This is a function that draws distributions in the "margins" (outsides) of the plot.  
     * The first argument takes the name of our pre-defined plot.  
