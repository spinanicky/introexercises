...
Open the R project "intro_course", as usual, by clicking on the R project icon in the "intro_course" folder. Open your script "ebola_analysis.R". You can locate this file by clicking the menus for *"File -> Open File", or by clicking your RStudio Files Pane (lower-right) and navigating to the file there.  
...
R does not necessarily remember which commands you ran in the previous session. In fact, it is better for reproducibility if R does *not* remember. This forces you to write scripts that are independent, and do not rely on any previous commands having been run.  

**Run all the commands in your script section-by-section**, to do the following:  

1) Load packages  
2) Import the "raw"/unclean surveillance linelist 
3) The cleaning pipeline ( %>% ) that produces the clean surveillance linelist  
4) Define the parameters such as `last_date_onset`  
5) Export the clean dataset  

If you have arranged your script correctly and kept it clean, these commands should all be able to be run in sequence with no interruptions.  

**Verify that you now have the data frame object `surv` in your R Environment, with rows and columns ready to be plotted**  

`r fontawesome::fa("bookmark", fill = "orange")` IF your script is malfunctioning and causing errors, you can write a command to import the clean `surv` dataset that you saved at the end of the previous module. You saved it as "surveillance_linelist_cleaned.rds" in the "data/clean" subfolder. You can import it with the command `import(here("data", "clean", "surveillance_linelist_cleaned.rds"))". After doing this, please notify your facilitator that you will need assistance cleaning your script so that it runs smoothly.  
...
**Now change the command so that `fill` is assigned to the column `district`.** There are two important steps:  
...
**If you did not move `fill = district` within the mappings, what error would you see in the R console?**  
...
**Lastly, try to adjust the `fill =` aesthetic.** Can you assign it to another column? In this new counts data frame, which columns are available to you?  
...
**Try to facet this histogram by `gender` (leave `fill = gender` as well).**  
...
Go to your **Help** pane in RStudio, and type in "gghighlight". Ensure that you have already installed the package.  

**Read the documentation.**

```{r quiz-gghighlight}
quiz(caption = "Quiz - gghighlight",
  question("When was the package documentation created?",
    allow_retry = T,
    answer("2021-01-01"),
    answer("1999-04-03"),
    ...
    question("The author believe's that this package is the equivalent of which function, but for ggplot2?",
    allow_retry = T,
    ...
    question("Select all the geoms have labels automatically added by gghighlight()?",
    allow_retry = T,
    answer("points", correct = T),
    answer("lines", correct = T),
    answer("bars", correct = T)
  )
)
```
