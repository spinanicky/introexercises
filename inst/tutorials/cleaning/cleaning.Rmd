...
quiz(caption = "Quiz - residence",
  question("How many cases were residents of Mountain Rural but were detected in Central II?",
    ...
    answer("4", correct = T, message = "residence is the rows, and detected is the columns"),
    answer("0", message = "residence is the rows, and detected is the columns"),
...
Remember - it may be tempting to exit and go clean up the dataset in the external CSV file... but in almost all circumstances it is more reproducible and more efficient to record the changes in the code. More than likely, you will receive another version of the "raw" data and all the cleaning work in the CSV file will be lost. However, if you have made the changes in the R script then you will have a record of all the adjustments.  
...
* Begin the command with the "raw" dataset  
* Pass it through a series of cleaning functions with the `%>%` pipe operator  
* Save the result as **`surv`** - a new, clean, data frame  

Because the command will use the assignment operator **`<-`**, the modified dataset will not be printed to the R Console. Instead, the changes will be reflected in the new object **`surv`**. To see the data frame you have two options:  
...
Here is a simplified example of cleaning code, with only a few (empty) functions. Note that this code will not run. In the coming sections of this exercise, we will build a chain like this together.   
...
* Change `onset_date` to `date_onset` (it is more standard to put "date" at the front, especially if there are multiple date columns)  
...
* Keep and rename certain columns (in one command)  

### Select columns to keep  

The primary purpose of `select()` is to select columns to keep in the dataset.  

Any column name that is not included in the parentheses will be dropped. Try the below command in your R console. It takes your partially-clean data set `surv` and selects only a few columns, printing (not saving) the result.    
...
`r fontawesome::fa("terminal", fill = "black")` Now time to code! Add to your pipe chain a step that removes the column `row_num` from the dataset. This column is not necessary for our analyses.  
...
**Add `distinct()` to the pipe chain and run it.
...
Within a pipe chain, it may may look like this:  
...
```{r, eval=F, echo=T}
head(surv)                # the first six rows
head(surv$date_onset)     # the first six values in the column date_onset
```
...
To create this column, we use the function `floor_date()` from the package {lubridate}, with the following argument settings:  

1) the column of dates (e.g. `date_onset`)  
2) `unit = "week"` (this can be set to other values such as "month")  

This will return a *date*, that is the Sunday prior to the onset or report date - the "floor" Sunday. This is the default.  
...
If you need to create epiweeks that begin on Monday, you can set the argument `week_start = 1`.  
...
The `mutate()` function is also used to *create* new columns. If the column name on the left side of the equals sign ( = ) does not yet exist, then it is created. As we have already done, if the column name on the right does already exist, then it is modified.  

**Try adding two `mutate()` commands that create two epiweek columns (`week_onset` and `week_report)`, from `date_onset` and `date_report` respectively. Make both using Mondays as the week start.**  
...
**Run this short code below to test the new command - it makes the change and prints the tabyl.** Do you see how the missing values are now listed as NA's?  
...
It is totally possible to write one `mutate()` command for each of these columns, but that would take a long time. Below we give you some advanced R code that automatically applies this change across all columns that are of class "character". Later, you can read more about `across()` in the [Epi R Handbook here](https://epirhandbook.com/en/cleaning-data-and-core-functions.html#clean_across).  
...
`r fontawesome::fa("bookmark", fill = "orange")` **Please note the below commands related to missingness in R**.  
...
Missingness in R is represented by `NA` - or at least it should be - as we discussed above. The function to assess missingness is `is.na()` (whereas `!is.na()` is used to test for non-missingness). In both of these functions, the object you want to assess in placed within the parentheses.  

To *count* the number of rows that meet a certain logical criteria, you can further wrap them in `sum()`, as below:  
...
**Run these two commands** (not in your pipe chain, just as stand-alone commands)  
...
In the weight column `wt_kg`, there are some negative values. Open the data frame in the viewer (e.g. by clicking the data frame name in the Environment pane) and sort the dataset by clicking on the `wt_kg` column.  

Run the {base} command `summary()` on the column `surv$wt_kg` to return key summary statistics on this numeric column:  

```{r, echo=T, eval=F}
summary(surv$wt_kg)
```

This must be some data entry error. To avoid errors in any future calculations or plots, convert these values to `NA` (missing). Remember, you should make edits like this here in the script. It is better than editing in CSV file, because here there is a record of changes performed.  
...
```{r, echo=T, eval=F}
  ...
  mutate(age_years = case_when(
    ...
    # logic on age_unit    # adjustment to make age_years  
    logic_if_months      ~ calculation,    # if age is given in months, return: age / 12
    logic_if_years       ~ calculation,    # if age is given in years, leave as: age 
    logic_if_missing ~   ~ age,            # if age unit is missing, assume years, and leave as: age
    TRUE                 ~ NA_real_))      # leave this unchanged. In any other circumstance, assign missing.
```


Of note, the last line should remain unchanged. It is best practice to end a `case_when()` command with `TRUE`, that will catch all rows that have not met any of the previous logical criteria. In this case we set this to `NA`.

It is important to know that in `case_when()`, all of the right-side values must be of the same class (numeric, integer, character, date, logical, etc). Sometimes, this requires assigning variations of `NA` such as `NA_character_`, `NA_real_` (for numeric), and `as.Date(NA)`. Read more in the dates chapter of [the Epi R Handbook](https://www.epirhandbook.com/en/working-with-dates.html). In this case our new variable will be numeric, so we use NA_real_.  
...
**Try adding this command to the pipe chain** To see if the code is correct, compare the values in `age_years` and `age_cat`.  
...
One way to assess how similar the columns are, is to *create a new column* that indicates whether the two values are different. Then, you can tabulate this new column.  
...
Use the manual **filters** in the dataset viewer to answer this question:  
...
`r fontawesome::fa("eye", fill = "darkblue")` **Review your cleaning pipe chain, and the one in the solution.** Ensure that all the cleaning steps in the solution are also present in your R script "ebola_analysis.R".    
...
Because saving as XLSX or CSV can cause to have to re-clean the data later, we save it as a ".rds" file. RDS is a file format specific to R, and is very useful if you know you will work with the exported data again in R. 
...
Congratulations! You have completed the cleaning exercise!  

Be sure to save your R script, and check-in with your facilitator.  

**In case you did not finish, do not worry.** *In the case that you can not finish, look in the "scripts/backup" folder. You will find R scripts for the Ebola analysis at every stage of this course. So you can always us these as a "backup".*  
