...
This will involve using previously explored aspects such as `ggplot()`, pipe chains `%>%`, themes (e.g. `theme_minimal()`, `theme_bw()`), custom axis scales (e.g. `scale_y_continuous()`) and new functions such as `geom_tile()` for heat plots, `incidence` (from **incidence2**) and `geom_histogram()` for epidemic curves, and `age_pyramid` (from **apyramid**) for age/sex pyramids.
...
Open your R project and run your entire "ebola_analysis.R" script, so that you are able to use the data frame `combined`. This will involve running all the parts of your R script, including to import data, clean, perform joins, and create the `combined` dataset.  
...
1) Use `combined` to create a new data frame named `age_outcome_summary`.  
  ...
  * Then, use `group_by()` and `summarise()` to show summary counts for each unique combination (group) of `age_cat10` and `gender`.  
...
In its most basic form, plotting epidemic curves in {ggplot2} is a case of using the function `geom_histogram()`. However, there are 3 main things to watch out for:  
...
`r fontawesome::fa("terminal", fill = "black")` Can you make a simple histogram from the data frame `combined` which shows the incidence of `date_onset` using 5-day intervals? The histogram "binwidth" can be specified as a static aesthetic to `binwidth = `. Also, clean up the labels and use `theme_bw()`.  
...
But typically we want the intervals to start on **Mondays** (or some other day), so we can add `floor_date()` and it's companion `ceiling_date()` :  
...
Let's replace them with `min()` and `max()` applied to the column `combined$date_onset`. As this is not a {dplyr} pipe, we need to specify the data frame and column with the `$` index operator. Don't forget that these two functions need `na.rm=TRUE` to escape missing values.  
...
Now if you run just `ebola_breaks`, the vector is returned/printed. So we can provide `ebola_breaks` to the `breaks = ` argument of `geom_histogram()`.  
...
ggplot(data = combined, aes(x = date_onset)) +
  geom_histogram(breaks = ebola_weeks) +
  labs(x = "Date of onset", y = "Incidence") +
  theme_bw()
...
`r fontawesome::fa("terminal", fill = "black")` Now it is your turn. Adjust the code so that the breaks are by *month* from the month-start prior to the first case, until the month-start after the last case. 

Once you have done this, can you create an epidemic curve in which the `fill` in the bars is by hospital, and the x-axis labels appear every month?
...
* To adjust the x-axis labels, add `scale_x_date()` and set `labels = date_label_easy()`
</br>
</details>
...
ggplot(data = combined, aes(x = date_onset)) +
  geom_histogram(breaks = ebola_months) +
  scale_x_date(
    date_breaks = "months",
    labels = scales::label_date_short())
  labs(x = "Date of onset", y = "Incidence") +
...
`r fontawesome::fa("terminal", fill = "black")` Can you look up the function (`?age_pyramid`) and then create an age pyramid from `combined` of `age_category` that is split by `gender`?
...
```{r quiz3}
quiz(
  question("Note the data format, which one of these is it?",
