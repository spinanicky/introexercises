...
1) the column of dates (`date_onset`)  
...
**Try adding another `mutate()` command that creates epiweek of report, using `date_report`.**  

This time, adjust your command so that you nest the creation of these two columns *within one `mutate()` command*. For example:  

```{r, echo=T, eval=F}
combined %>% 
  mutate(col1 = floor_date(___),
         col2 = floor_date(___))
```
