...
* Can you create a new object, "hospital_week_adjusted" where the NA values in `hospital` are recoded as "Missing"?
* We would also like to re-order the y-axis, from top to bottom, to read "Central Hospital", "Military Hospital", "Port Hospital", "SMMH", "Other", `NA`.
* Change the color scheme to a diverging colour scale, could you provide one that goes from "blue" at low values to "red" at high values
* The labels should be cleaned up

This will require a mix of refactoring the dataset, and adjusting your **ggplot** function.
