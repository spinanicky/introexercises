...
Prior to this course, we held sent you a guide to help you install R, RStudio, and the important R packages for this course. If this was not successful, please contact one of our instructors immediately.

If you already had R installed, please update to the most recent version for the course. This involves re-installing R.  
...
`r fontawesome::fa("window-restore", fill = "darkgrey")` Open RStudio from your desktop or computer's list of applications. As it opens, RStudio will automatically find and use R. There will almost never be a need to open R directly. 
...
1) Click through to *File -> New Project*. Select **"Existing directory"**.  
...
3) **Click "browse" and select the "intro_course" folder that you downloaded and which contains the course materials**.  

`r fontawesome::fa("exclamation", fill = "red")`  

>For this course, do NOT save the intro_course folder/R project to a shared drive such as Sharepoint, OneDrive, Google Drive, etc. Save it to your desktop or a drive that is local to your computer only. While hosting projects on drives like this is possible (and important for team work), it requires some testing and coordination between team members.    

4) Check the box "Open in new window"  

5) Click "Create project"  

Voila! A new RStudio window should open. Click on that window and look at it. It should look very similar to your old one, but see if the upper-right corner of this new RStudio shows that it is in the "intro_course" RStudio project.  


### Explore your project  

**Close this new RStudio window, but leave your old one (with this exercise) open.** If asked to save a 'workspace image", say NO (we'll explain why later).  

`r fontawesome::fa("window-restore", fill = "darkgrey")`  **Open your File Explorer.** Navigate to where you saved the R project. *What do you see?*  

If you saved the folder to your desktop, the file explorer would look like this:  
...
**Now, double-click the RStudio Project file** to re-open your "intro_course" project. RStudio should open.  
...
Until now, this exercise has been running from the RStudio window that is *not* associated with your RStudio project. At this moment, you should have two RStudio windows open:  

1) The RStudio window *not associated with your "intro_course" RStudio project*, but is running this exercise  
2) The RStudio window *that is associated with the "intro_course" project* but it not running any exercise  

To simplify your workflow and de-clutter your screen, we will have you close this RStudio window (close this exercise) and re-open the exercise in your new RStudio window.  

**First, open this same tutorial in your new RStudio window, using the image below as a guide.**  

1) View the new RStudio window  
2) Click open the "Tutorials" Pane in the upper-right  
3) Click to start the exercise "Applied Epi - R setup, syntax, data import"  
4) Navigate in the tutorial to this same section ("Switch to your RStudio project")  
...
knitr::include_graphics("images/switch_exercise_windows.png")
```

**Once you are established in the new RStudio window, stop this exercise by pressing the red STOP icon in the upper-left of the exercise.  Then close this old RStudio window.**  

You can now continue the exercise in your RStudio project.  
...
Time for you to practice in RStudio. In your R script (named "demo.R"), use the function `min()` and `max()` to find the minimum and maximum of the numbers 3, 55, 9, -4, and 33.  
...
**Cut the file *"surveillance_linelist_12012014.csv"* from the root folder, and paste it into the "data/raw" subfolder.**  
...
Place the name of the linelist in `skim()` and run. What content appears in the R console? If it is not easily readable, try expanding your Console pane to be larger, and run the command again.  
