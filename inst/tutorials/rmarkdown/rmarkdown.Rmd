...
```{r, include=FALSE}
# data prep --------------------------------------------------------------------
surv_raw <- rio::import(system.file("dat/surveillance_linelist_12012014.csv", package = "introexercises"))
```
...
knitr::include_graphics("images/1_gettingstarted.png")
```
...
knitr::include_graphics("images/4_knitbutton.png")
```
...
An HTML file can be emailed as a static file, similar toa PDF or Word document. When the recipient receives it, the file will open in the internet browser - but the file is not online! It is only saved locally on their computer, but the internet browser is used to display the file. This is important to know, if you are concerned about sharing sensitive information in an HTML report.  

*Note, it is also possible to post an HTML file online, but this is done through a different process*  

### Word  

**Now click the "knit" button again, but use the drop-down to knit as Microsoft Word**  

### PDF  

Knitting to PDF is more difficult and you may encounter errors and challenges with formatting. Nevertheless, try to knit directly to PDF. If you encounter errors, do not worry. We will not stop now to deal with them. You can discuss them with your instructor during some spare time. *Remember, you can always knit to Word, and then convert to PDF.*  

### Other formats  

Many other formats exist. You can try Powerpoint by clicking "Presentation" in the window that appeared when you opened the Rmd for the first time.  
...
`r fontawesome::fa("eye", fill = "darkblue")`  
Note the section in the top - the "YAML". This section is delimited by three dashes on its top and bottom 
...
Edit the YAML to look exactly like this (with your name in the `author:` section):  
...
Note that we have removed the `pdf_document` and `html_document` output options for clarity. In this exercise we will build a Word document report, so that it is easy to edit after rendering.  


### Params  

Add further lines to your YAML as shown below. These will be the `params` list for your report. 
...
knitr::include_graphics("images/3_chunk.png")
```
...
One particularly fun option is "code folding", which is only available for HTML reports.  

1) Edit the `setup` chunk at the top so that `echo = TRUE`  

2) Edit your YAML to look like below (note the change to `output:` to `html_document:` with a colon after it. And then the indenting to `code_folding: hide`)  
...
**Change your `setup` default options to as below.**  
...
```{r, eval = TRUE, echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/2_text.png")
```

**Using the image above as a guide, modify the default text to show the text below:** Note that to make a newline you need to add **two spaces and hit Enter** at the end of each bullet/sentence.  

Many people enjoy learning R because they seek:  

* Analytical *flexibility*  
* Advanced *graphics* and automated *reporting capability*  
* Above all to **support the movement towards free, open-source software**  



## Build your report  

Now that you know about YAML, knitting, code chunks, text, and inline text... begin to make your Ebola situation report!  
...
7) Create and print plots for *person*, *time*, and *place*  

Do not hesitate to ask your facilitator for assistance if you get stuck!  

**Do not forget to save your file as you do this!**  
...
## Make your report adaptable  

If you finish creating your report and still have time before the class re-convenes in the plenary debrief, follow the instructions below to make your report adaptable as new data arrive.  
...
**Let's use this threshold**: Only include cases that were reported (column `report_date`) **on or before** the Sunday of the week **prior** to the `publish_date` set in the YAML.  

Ok, let's begin.  

#### Class of the param `publish_date`  

You have already created the `params` value `publish_date`, which can be accessed within the report by typing `params$publish_date`.  

But what class is this value? Try running just `params$publish_date` in the R Console. Then run `class(params$publish_date)`. What class is this? What function should you apply to convert it to a date?  
...
We can return the Sunday prior to a date using `floor_date(DATE, unit = "week")` from the {lubridate} package. To return the Sunday of the *prior* week, we can just subtract 7 days from the date: `floor_date(DATE - 7, unit = "week")`  
...
Add captions to each plot using `labs(caption = )` and make the character value dynamically describe the last date eligible for cases to be included in the plot. For example:  
...
If you have extra time, try to make your report look nicer with the following options.  
...
```{r, eval = TRUE, echo = FALSE, out.width = "50%"}
knitr::include_graphics("images/9_html.png")
```
