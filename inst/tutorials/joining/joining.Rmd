...
The most common joins in public health contexts are:  
...
quiz(caption = "Which join would you use:",
     ...
     question("Which of these joins would use patients as the baseline?",
              answer("left_join(patients, results, by = 'ID')", correct = TRUE),
              answer("right_join(results, patients, by = 'ID')", correct = TRUE),
              answer("left_join(results, patients, by = 'ID')"), correct = FALSE,
              allow_retry = TRUE),
...
test <- full_join(surv, hosp,
          by = c(
            "case_id" = "ID",
            "age",                    # columns "age" and
            "age_unit",               # "age_unit" are named the same in both 
            "gender" = "gender",
            "hospital" = "hospital"))
```
...
3) Now, run a `left_join()` on `surv` and `hosp`, using "case_id" as the common identifier  
...
Click on the `lab` data frame in the Environment pane. Review the columns. Are there any common columns between this data frame and `surv` that we can use to join lab data to the survillance linelist?  
...
How would you perform a *left join* of `df_patient` with `lab` (where `df_patient` is used as reference/baseline dataframe)?
...
Because we want `lab` as the baseline, it should be listed first. Do not forget the by=

```{r, echo=T, eval=F}
anti_join(lab, surv, by = "ID")
```
