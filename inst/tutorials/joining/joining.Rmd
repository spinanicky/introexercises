...
surv_small <- rio::import(system.file("dat/surveillance_linelist_clean.rds", package = "introexercises")) %>% 
...
Use `clean_names()` first to standardize the syntax. Then use `rename()` to manually adjust specific column names (new = old). To establish date class, choose between `mdy()`, `ymd()`, or `dmy()` to place within the `mutate()` function, for example:  `mutate(date_outcome = mdy(date_outcome))`
...
combined <- combined %>% 
  ...
  mutate(date_hospitalisation = mdy(date_hospitalisation),
         ...
         date_infection       = mdy(date_infection))
```
...
quiz(caption = "Quiz - Final cleaning",
  ...
  question("Why did we choose mdy() to clean the dates?",
    allow_retry = T,
...
Take a moment to ensure that your script aligns with the above
...
If you have extra time, complete the extra sections below.  
